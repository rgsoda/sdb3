package CustomerList;
use warnings;
use strict;
use Log;
use Data::Dumper;
use Gtk2::SimpleList;
use notebook_tab;
use Popup;
use Customer::EditCustomer;
use Customer::CustomerDB;
our @ISA = ('notebook_tab');

sub new {
	my $class = shift;
	my $self = $class->SUPER::new(shift,'customer List');
	
	$self->{list} = new Gtk2::SimpleList( 	'id' 		=> 'hidden',
						'Company'	=> 'text',
						'Country'	=> 'text',
						'Address'	=> 'text',
						'City'		=> 'text',
						'NIP'		=> 'text',
						'Regon'		=> 'text',
						'Email'		=> 'text',
						'Contact Person'		=> 'text',
						'Phone'		=> 'text',
						);
	@{$self->{list}->{data}} = @{CustomerDB->get_customer_list()};
	$self->{list}->signal_connect(row_activated => sub {
		$self->{edit} = new EditCustomer($self->{tab},$self->{list}->{data}[$_[1]->to_string][0]);
	});
	$self->{win}->add_with_viewport($self->{list});
	$self->{win}->show_all();
}

