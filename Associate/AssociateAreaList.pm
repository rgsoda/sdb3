...
use constant AREA_VALUES => ('dyehouse','knittinghouse','spedition','stockhouse');
...
sub row_activated {
	...
	$self->{entry}->{valid} = 1;
	$self->{entry}->set_text($self->{widget}->{data}[$_[1]->to_string][0]);
	$self->close;
}
