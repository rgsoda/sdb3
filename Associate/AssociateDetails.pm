...
use Gtk2::PopupEntry;
use Gtk2;
use Associate::AssociateAreaList;
...
sub new {
	...
	$self->{area_entry} = new Gtk2::PopupEntry(\$self->{data_hash}->{area},undef,'Area','AssociateAreaList');
	$vbox->pack_start($self->{area_entry},1,1,0);
	
	$self->{company_entry} = new Gtk2::ValidEntry(\&Validators::text_valid,'Company',1);
	$vbox->pack_start($self->{company_entry},1,1,0);
